...
# 70% people think climate change affect their community in this sample.
```

2.  Would you expect another student's sample proportion to be identical to yours? 
    Would you expect it to be similar? Why or why not?

**It wouldn't be identical because the surveyed individuals can't be exactly the same. But the results should be close to the true proportion as the sample size gets larger.**
...
**For example, the sample runs 1000 times, building 1000 intervals. About 950 intervals cover the true proportion, 50 will miss. So let's say 95% confident that the true proportion is between 0.567 and 0.817.**
...
**Yes it captures (0.567-0.817). My neighbor may or may not capture the value depending on their random sample.**
...
**About 95% of the intervals should capture the true population mean since that's what a 95% confidence level mean.**
...
**There're 94% intervals captured the true proportion. It's not exactly equal to the confidence level. It's plausible because there are 50 intervals only. In the long run 95% of intervals will include the true value. Also, 94% is within the acceptable variance.**
...
**The confidence level gets narrower if it decreases. Because when the intervals get narrower, it's harder to capture the true value and that's why the confidence level is lower.**
...
# I choose 90% ci here, which means it allows more missing 
# values (10% will miss). It's less likely to capture true
# proportion.
```
...
**I changed ci to 90% and the missing intervals fluctuated between 6 or 7, which meant 88% or 86% were actually accurate. So there was 2% or even 4% error. It was much larger than the 1% error that we got when the ci was 95%.**
...
# The interval is [0.617,0.783]. It's really close to 0.62.
# I run 85% several times and I got the missing intervals
# like 7 or 8. The actual ci level could be a bit lower.
# Low ci level is trading off accuracy for precision.
```
...
**I don't see obvious changes when the sample size changes.**
...
**The width of the interval increases as the number of bootstrap increases. So does the standard error.**
