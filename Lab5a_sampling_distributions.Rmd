...
# The sample results are 0.74 for Benefits and 0.26 for non-Benefits.  
# They are slightly different with the population probability but acceptable.
```
...
**The sample proportion would be somewhat different because the individuals surveyed would be different. However, the sample proportion should be close to the true proportion of the population as long as the sample sizes are large enough and random.**
...
# sample_n(50): 0.84 and 0.16
# sample_n(100): 0.81 and 0.19
# sample_n(1000): 0.798 and 0.202
# The estimate is more accurate as the size gets bigger.
```
...
```{r fig.show="hide"}
# I guess I can just copy the plot
...
# The elements in this plot include title and subtitle, grid lines
# x-axis with scale 0-2500 counts, y-axis with scale 0.0-0.45, 
# bars. The center is 0.2.
```
...
# Each obs. in this data frame means the result of one trial 
# showing how many people said 'Doesn't benefit' and the proportion # of it.
```
...
**Each observation represents the probability that the surveyed people said "Doesn't benefit" in the sample size of 10, 50 and 100.  As the sample size increases, the mean will get closer to 0.2, SE gets closer to 0.0 and the shape gets narrower to 0.2.  These values don't get affected as the number of simulations increases.**
...
# The best point estimate here is 0.733
```
...
sample_props15 <- global_monitor %>%
  ...
  count(scientist_work)%>%
  mutate(p_hat = round(n/sum(n),1)) %>%
  filter(scientist_work == "Benefits")

ggplot(data = sample_props15, aes(x = p_hat)) + geom_histogram(binwidth = 0.05) + 
  labs(
    x = "p_hat (Benefits)",
    title = "Sample Distribution of P_hat",
    subtitle = "Sample size = 15, number of sample = 2000"
  )

# The plot is symmetrical centered at 0.8 and that seems like
# the true proportion of "Benefits".

global_monitor %>%
  count(scientist_work)%>%
  mutate(p = n/sum(n)) # Benefits 0.8
```
...
ggplot(data = sample_props150, aes(x = p_hat)) + geom_histogram(binwidth = 0.05) + 
  labs(
    x = "p_hat (Benefits)",
    title = "Sample Distribution of P_hat",
    subtitle = "Sample size = 150, number of sample = 2000"
  )

# In this case, there are only three columns 0.7,0.8 and 0.9 
# while 0.8 is dominant. Sample_15 have wider range of x-axis.
# Base on this plot, 0.8 is true proportion.
```
...
**Sample_150 has smaller spread.  If the goal is to look for the true value, then a small spread is preferred, namely, larger sample size.**
